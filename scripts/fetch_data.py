...
STRATEGY_A = [
    ...
    {"ticker": "IUSV.L",   "name": "iShares Core S&P US Value",       "role": "Value USA LC"},
    {"ticker": "CBUX.L",   "name": "iShares Global Infrastructure",   "role": "Infrastructure"},
...
STRATEGY_B = [
    ...
    {"ticker": "CBUH.L",   "name": "iShares MSCI World Momentum Adv", "role": "World Momentum"},
    {"ticker": "XEON.DE",  "name": "Xtrackers EUR Overnight Rate",    "role": "Cash"},
]
...
def calc_returns(prices: pd.Series) -> dict:
    ...
    return {
        ...
        "6m":  ret(126),
        "1y":  ret(252),
        "ytd": round((today_price / prices[prices.index >= f"{TODAY[:4]}-01-01"].iloc[0] - 1) * 100, 2)
                if any(prices.index >= f"{TODAY[:4]}-01-01") else None,
    }
